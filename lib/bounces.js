...
// Both take a plain, protocol-agnostic shape — {from, subject, contentType,
// text} — rather than a mailparser-specific object, so a REST-based provider
// (Zoho, which has no MIME message to parse, just JSON fields) can build the
// same shape from its own response and reuse this exact detection/parsing
// logic instead of forking a copy that will drift.
function looksLikeBounce({ from = '', subject = '', contentType = '' }) {
  const f = from.toLowerCase();
  const s = subject.toLowerCase();
  const ct = contentType.toLowerCase();
  if (/mailer-daemon|postmaster|mail delivery (system|subsystem)/.test(f)) return true;
  if (ct.includes('multipart/report')) return true;
  if (/undeliver|delivery status notification|delivery (has )?failed|returned mail|failure notice|delivery incomplete/.test(s)) return true;
  return false;
}

function extractBounceInfo({ text = '' }) {
  const blob = text;
...
// failure, the mailbox is just where the signal comes from. `pass` and
// `accessToken` are mutually exclusive — pass whichever the account actually
// has (a plain password for preset/custom-SMTP accounts, an OAuth2 access
// token for Gmail, both authenticated the same way at the IMAP protocol
// level via imapflow's `auth` option).
async function findBounces({ host, port, secure, user, pass, accessToken, since }) {
  if (!host || !user || !(pass || accessToken)) return [];
  ...
  try {
    ...
    try {
      ...
      for (const uid of uids || []) {
        ...
        // The machine-readable RFC 3464 delivery-status part (when present)
        // carries the Final-Recipient/Diagnostic-Code fields the regexes
        // look for — merge it into the plain-text body before normalizing,
        // since that MIME-attachment structure is specific to this IMAP
        // path (a REST provider's JSON response has no such thing).
